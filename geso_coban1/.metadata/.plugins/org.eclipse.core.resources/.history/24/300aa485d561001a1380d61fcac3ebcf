...
public class DonBanHangInPdf_HangSvl extends HttpServlet{
	private static final long serialVersionUID = 1L;

    public DonBanHangInPdf_HangSvl() {
    	super();
    }
	...
	private void CreatePDF( HttpServletRequest request,HttpServletResponse response,IDonBanHang_Hang obj) {
		
		//định nghĩa kiểu trả về cho response là pdf
		response.setContentType("application/pdf");
		//đặt tên cho file của bạn
		response.setHeader("Content-Disposition"," inline; filename=DonBanHangInPdf_Hang.pdf");
	...
	private void FillData(Document document, ServletOutputStream outstream, 
			HttpServletResponse response, HttpServletRequest request, dbutils db,IDonBanHang_Hang obj){
		try
		{		
			...
			float[] crSanpham = {1.1f * CONVERT, 1.8f * CONVERT, 2.7f * CONVERT, 1.7f * CONVERT, 2.0f * CONVERT,
			...
			String[] spTitles = {"STT", "Mã sản phẩm","Tên sản phẩm", "Đơn vị", "Số lượng", "Số lượng tồn", "Đơn giá", "Thành tiền"};
			...
			for(int i=0;i<listsp.size();i++){
				IChiTietDonHang_Hang ctdh = listsp.get(i);
				...
				/*String[] spTitles2 = {""+sott, ctdh.getMaSanPham(), ctdh.getTenSanPham(),ctdh.getDonViTinh(),
						...
						formatter_2sole.format(ctdh.getDonGia()), formatter_2sole.format(ctdh.getThanhTien()) } ;*/
				
				String[] spTitles2 = {""+sott, ctdh.getMaSanPham(), ctdh.getTenSanPham(),ctdh.getDonViTinh(),
						formatter_2sole.format(ctdh.getSoLuong()) , formatter_2sole.format(ctdh.getSoLuongTon()) ,
						formatter_2sole.format(ctdh.getDonGia()), formatter_2sole.format(ctdh.getSoLuong()*ctdh.getDonGia()) } ;
