...
public class DoanhSoTheoTinhSvl extends HttpServlet
{
	private static final long serialVersionUID = 1L;
       
    public DoanhSoTheoTinhSvl() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		String nextJSP = "/TrainingGESO/pages/Erp/DoanhSoTheoTinh.jsp";
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
	    ...
	    else
	    {
			...
			response.setHeader("Content-Disposition", "attachment; filename=DoanhSoTheoTinh.xlsm");
    ...
    {   
		...
		fstream = new FileInputStream(getServletContext().getInitParameter("path") + "\\DoanhSoTheoTinh.xlsm");
	    ...
	    worksheet.setName("Sheet1");
	     ...
	     CreateStaticHeader(workbook, obj ,worksheet );	     
	     boolean isTrue = CreateStaticData(workbook, obj, condition,worksheet);
	     if(!isTrue){
	    	 return false;
	     }
	     Worksheet worksheet2 = worksheets.addSheet();
	   	   
	     worksheet2.setName("TINH");
	      CreateStaticHeader1(workbook, obj ,worksheet2 );	     
		       isTrue = CreateStaticData1(workbook, obj, condition,worksheet2);
		     if(!isTrue){
		    	 return false;
		     }
	...
	private void CreateStaticHeader(Workbook workbook,Ireport  obj,Worksheet worksheet) 
	{
	    ...
	    String tieude = "BÁO CÁO DOANH SO TUNG MAT HANG THEO TINH";
	    ...
	    ReportAPI.getCellStyle(cell, Color.NAVY, true, 9, "Được tạo bởi:  " + UserName);
			...
			cell = cells.getCell("A5");cell.setValue("Mã Tỉnh");ReportAPI.setCellHeader(cell);
			cell = cells.getCell("B5");cell.setValue("Tỉnh Thành");ReportAPI.setCellHeader(cell);
			cell = cells.getCell("C5");cell.setValue("T1");ReportAPI.setCellHeader(cell);
			cell = cells.getCell("D5");cell.setValue("T2");ReportAPI.setCellHeader(cell);
			cell = cells.getCell("E5");cell.setValue("T3");ReportAPI.setCellHeader(cell);
			cell = cells.getCell("F5");cell.setValue("T4");ReportAPI.setCellHeader(cell);
			cell = cells.getCell("G5");cell.setValue("T5");ReportAPI.setCellHeader(cell);
			cell = cells.getCell("H5");cell.setValue("T6");ReportAPI.setCellHeader(cell);
			cell = cells.getCell("I5");cell.setValue("T7");ReportAPI.setCellHeader(cell);
			cell = cells.getCell("J5");cell.setValue("T8");ReportAPI.setCellHeader(cell);
			cell = cells.getCell("K5");cell.setValue("T9");ReportAPI.setCellHeader(cell);
			cell = cells.getCell("L5");cell.setValue("T10");ReportAPI.setCellHeader(cell);
			cell = cells.getCell("M5");cell.setValue("T11");ReportAPI.setCellHeader(cell);
			cell = cells.getCell("N5");cell.setValue("T12");ReportAPI.setCellHeader(cell);
	...
	private boolean CreateStaticData(Workbook workbook, Ireport obj, String condition,Worksheet worksheet) throws Exception
	{
		...
		String	query=" select  TT.PK_SEQ AS MATINH ,TT.TEN AS TINHTHANH ,DH.* from(  "+ 
					 " select  tinhthanh_Fk, CAST(a.thang AS INT) AS THANG,sum(a.thanhtien ) as thanhtien from (  "+ 
					 " 	select   npp.TINHTHANH_FK   , substring(dh.NgayDonHang,6,2)  as thang ,SUM( dhsp.soluong*dhsp.dongia ) as thanhtien  "+ 
					 " 	from ERP_DONDATHANGNPP  dh  "+ 
					 " 	inner join ERP_DONDATHANGNPP_SANPHAM dhsp on dh.PK_SEQ=dhsp.dondathang_fk  "+ 
					 " 	 INNER JOIN NHAPHANPHOI NPP ON NPP.PK_sEQ=DH.NPP_DAT_FK  "+ 
					 "  "+ 
					 " 	where dh.TRANGTHAI!=3 and dh.NgayDonHang >='"+obj.gettungay()+"' and dh.NgayDonHang <='"+obj.getdenngay()+"'  "+ 
					 " 	group by npp.TINHTHANH_FK ,substring(dh.NgayDonHang,6,2)  "+ 
					 "  	union all  "+ 
					 " 	SELECT npp.TINHTHANH_FK ,substring(dh.NGAYTRA,6,2)  , (-1) *SUM( DHSP.SOLUONG*DHSP.DONGIA ) AS THANHTIEN  "+ 
					 " 	FROM ERP_HANGTRALAINPP  DH  "+ 
					 " 	INNER JOIN ERP_HANGTRALAINPP_SANPHAM DHSP ON DH.PK_SEQ=DHSP.HANGTRALAI_FK  "+ 
					 " 	 INNER JOIN NHAPHANPHOI NPP ON NPP.PK_sEQ=DH.npptra_fk "+ 
					 " 	WHERE DH.TRANGTHAI=1 AND DH.DOITUONG=0  AND DH.NGAYTRA >='"+obj.gettungay()+"' AND DH.NGAYTRA <='"+obj.getdenngay()+"'  "+ 
					 " 	GROUP BY npp.TINHTHANH_FK,substring(dh.NGAYTRA,6,2)  "+ 
					 " )  a  "+ 
					 " group by tinhthanh_Fk,thang  "+ 
					 "  "+ 
					 "  ) as data  "+ 
					 "  PIVOT  "+ 
					 " (  "+ 
					 ...
					 " ) AS DH	  "+ 
					 "  LEFT JOIN TINHTHANH TT ON TT.PK_SEQ=TINHTHANH_FK  "+ 
					 " 	 ORDER BY TT.TEN ";
		...
		int i = 6;
			...
			try
			{
				...
				while (rs.next())
				{
						cell = cells.getCell("A" + Integer.toString(i));cell.setValue(rs.getString("MATINH"));
						cell = cells.getCell("B" + Integer.toString(i));cell.setValue(rs.getString("TINHTHANH"));					
						cell = cells.getCell("C" + Integer.toString(i));cell.setValue(rs.getDouble("1"));
						cell = cells.getCell("D" + Integer.toString(i));cell.setValue(rs.getDouble("2"));
						cell = cells.getCell("E" + Integer.toString(i));cell.setValue(rs.getDouble("3"));
						cell = cells.getCell("F" + Integer.toString(i));cell.setValue(rs.getDouble("4"));
						cell = cells.getCell("G" + Integer.toString(i));cell.setValue(rs.getDouble("5"));
						cell = cells.getCell("H" + Integer.toString(i));cell.setValue(rs.getDouble("6"));
						cell = cells.getCell("I" + Integer.toString(i));cell.setValue(rs.getDouble("7"));
						cell = cells.getCell("J" + Integer.toString(i));cell.setValue(rs.getDouble("8"));
						cell = cells.getCell("K" + Integer.toString(i));cell.setValue(rs.getDouble("9"));
						cell = cells.getCell("L" + Integer.toString(i));cell.setValue(rs.getDouble("10"));
						cell = cells.getCell("M" + Integer.toString(i));cell.setValue(rs.getDouble("11"));
						cell = cells.getCell("N" + Integer.toString(i));cell.setValue(rs.getDouble("12"));
	...
	private void CreateStaticHeader1(Workbook workbook,Ireport  obj,Worksheet worksheet) 
	{
		 String dateFrom= obj.gettungay(); String dateTo =obj.getdenngay(); String UserName=obj.getuserTen();
	...
	private boolean CreateStaticData1(Workbook workbook, Ireport obj, String condition,Worksheet worksheet) throws Exception
	{
		dbutils db = new dbutils();
		  ...
		  Cells cells = worksheet.getCells();
			...
			param[1] = obj.getdenngay().equals("") ? null : obj.getdenngay();
			param[2] = obj.getuserId().equals("") ? null : obj.getuserId();
			String	query="	select  isnull(TT.PK_SEQ,0)  AS MATINH , isnull(TT.TEN,'')  AS TINHTHANH ,ISNULL(NPP.MAFAST, '') as Ma ,NPP.TEN as Ten ,DH.* from( "+
