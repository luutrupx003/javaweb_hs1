...
public class DonBanHangExcel_ThuSvl extends HttpServlet {

	private static final long serialVersionUID = 1L;

	public DonBanHangExcel_ThuSvl() {
	}
	...
	protected void doGet(HttpServletRequest request,
		...
		// khởi tạo đối tượng để truyền đi
		...
		if (action.equals("excel")) {
			...
			response.setHeader("Content-Disposition",
					"attachment; filename=InDonHangExcel_Thu.xls");
	...
	private boolean FillData(Workbook workbook, IDonBanHang_Thu obj){
		...
		try {
	        ...
	        List<IChiTietDonHang_Thu> ctdh= obj.getDanhSachSanPham();
	        ...
	        for(int i= 0; i< ctdh.size(); i++ ){
	        	...
	        	fillChiTietDonHang(index, "E", ctdh.get(i).getMaDonVi(), cell, cells, style);
	        	//đơn giá
	        ...
	        //làm ô tổng tiền
	        cells.merge(index-1, 1, index-1, 6);
	...
	private void fillChiTietDonHang(int chiso, String chucai, String noidung, Cell cell, Cells cells, Style style){
		...
		if(chucai.equals("F") || chucai.equals("G") || chucai.equals("H"))
		{
