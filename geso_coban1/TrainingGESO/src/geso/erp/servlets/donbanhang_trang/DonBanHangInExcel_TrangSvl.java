...
import geso.erp.beans.donbanhang_trang.imp.*;
...
public class DonBanHangInExcel_TrangSvl extends HttpServlet{
	private static final long serialVersionUID = 1L;

    public DonBanHangInExcel_TrangSvl() {
    	super();
    }
	...
	throws ServletException, IOException {
	 ...
	 // khai báo lớp tiện ích Utility
	    ...
	    if(action.equals("excel")){
			...
			response.setHeader("Content-Disposition", "attachment; filename=DonBanHangInExcel_Trang.xls");
	...
	private boolean CreateExcel( OutputStream out,HttpServletRequest request,HttpServletResponse response,IDonBanHang_Trang obj) throws IOException{
		...
		fstream = new FileInputStream(getServletContext().getInitParameter("pathTrang") + "\\DonBanHangInExcel_Trang.xls");
	...
	private boolean FillData(Workbook workbook, IDonBanHang_Trang obj){
		dbutils db = null;
		try {
			db = new dbutils();
			...
			cell.setValue("Nguyễn Bĩnh Khiêm");
	        ...
	        List<IChiTietDonHang_Trang> ctdh = obj.getDanhSachSanPham();
	        int index = 14;
	        for(int i= 0; i< ctdh.size(); i++ ){
	        	...
	        	System.out.println("SỐ LƯỢNG: " + cell.getValue());
	        	//số lượng tồn
